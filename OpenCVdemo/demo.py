...
def main():
    ...
    cv2.namedWindow('Demo tracking', cv2.WINDOW_NORMAL)	
    cv2.resizeWindow('Demo tracking', int(VideoSource.new_size[0]), int(VideoSource.new_size[1])+48)
    ...
    while (True):
        ...
        if VideoSource.nb_frame % cf.DISPLAYED_FRAME == 0:
            ...
            # t['imshow'] = time.time()
            cv2.imshow('Demo tracking',frame_annotation)
