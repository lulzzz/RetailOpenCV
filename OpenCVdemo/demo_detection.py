...
def main():
    ...
    if args.input:
        ...
        input_video = "dataset\\street960.mp4"
    ...
    cv2.namedWindow('Demo tracking (ECHAP pour quitter)', cv2.WINDOW_NORMAL)	
    cv2.resizeWindow('Demo tracking (ECHAP pour quitter)', int(VideoSource.new_size[0]), int(VideoSource.new_size[1])+48)

    logo_file = cv2.imread(cf.LOGO_FILE)
    init_file = cv2.imread(cf.INIT_FILE)

    #car_cascade = cv2.CascadeClassifier("car_cascade_10.xml")
    # car_cascade = cv2.CascadeClassifier("cascade\\cascade_10_5024_10k20k_haar.xml")
    #ciseaux_cascade = cv2.CascadeClassifier("cascade\\cascade_ciseaux_20_4040_LBP.xml")
    #main loop
    ...
    while (True):
        ...
        if (VideoSource.nb_frame >= cf.TRAIN_FRAMES):
            ...
            #30 14            
            #cars = car_cascade.detectMultiScale(gray, 4, 5)
            
            #cars = car_cascade.detectMultiScale(gray, 2,2)
            #cars = car_cascade.detectMultiScale(gray, 2, 8)

            #cars = car_cascade.detectMultiScale(gray, 10, 12)
            #for x,y,w,h in cars:
                #cv2.rectangle(frame_annotation, (x,y), (x+w, y+h), (0,255,0), 2)
                #print("CAR DETECTED {} {} {} {} {}".format(VideoSource.nb_frame, x,y,w,h))
            '''
            ciseaux = ciseaux_cascade.detectMultiScale(gray, 1.2)
            for x,y,w,h in ciseaux:
                cv2.rectangle(frame_annotation, (x,y), (x+w, y+h), (0,0,255), 2)
            ...
            if len(contours) > 0:
                ...
                if len(contours) > 0:
                    ...
                    if (len(temp_persons_detected_on_current_frame) > 0) & (len(temp_persons_detected_on_current_frame) < 20):
                        ...
                        if (VideoSource.nb_frame > 200):
                            for j,p in enumerate(persons):
                                if p.alive:
                                    ...
                                    if w > 80:
                                        cv2.imwrite("exports\img_"+str(VideoSource.nb_frame)+"_"+p.puuid+".png", cv2.cvtColor(frame[y:y+h, x:x+w], cv2.COLOR_BGR2GRAY))
                        '''
                        
            #tl.generate_negatives(VideoSource, persons)
        ...
        if VideoSource.nb_frame % cf.DISPLAYED_FRAME == 0:
            ...
            # t['imshow'] = time.time()
            cv2.imshow('Demo tracking (ECHAP pour quitter)',frame_annotation)
